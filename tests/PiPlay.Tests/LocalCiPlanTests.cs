...
public class LocalCiPlanTests
{
    ...
    public async Task Plan_is_side_effect_free_and_pins_the_shared_command_contract()
    {
        ...
        Assert.Equal("powershell.exe",
            first.GetProperty("requirements").GetProperty("windowsPowerShell").GetString());
        ...
        Assert.Equal(new[]
            {
                "node-version", "dotnet-info", "restore",
                "stable-root-policy-pwsh", "deploy-swap-policy-pwsh",
                "stable-root-policy-windows-powershell", "deploy-swap-policy-windows-powershell",
        ...
        Assert.Equal("pwsh", steps[3].GetProperty("filePath").GetString());
        Assert.Equal("Test-StableDeployRoot.ps1", Path.GetFileName(Arguments(steps[3])[2]));
        Assert.Equal("pwsh", steps[4].GetProperty("filePath").GetString());
        Assert.Equal("Test-DeploySwap.ps1", Path.GetFileName(Arguments(steps[4])[2]));
        Assert.Equal("powershell.exe", steps[5].GetProperty("filePath").GetString());
        Assert.Equal(new[] { "-NoProfile", "-ExecutionPolicy", "Bypass", "-File" },
            Arguments(steps[5])[..4]);
        Assert.Equal("Test-StableDeployRoot.ps1", Path.GetFileName(Arguments(steps[5])[4]));
        Assert.Equal("powershell.exe", steps[6].GetProperty("filePath").GetString());
        Assert.Equal(new[] { "-NoProfile", "-ExecutionPolicy", "Bypass", "-File" },
            Arguments(steps[6])[..4]);
        Assert.Equal("Test-DeploySwap.ps1", Path.GetFileName(Arguments(steps[6])[4]));
        Assert.Equal("pwsh", steps[7].GetProperty("filePath").GetString());
        Assert.Equal("Test-Documentation.ps1", Path.GetFileName(Arguments(steps[7])[2]));
        AssertStep(steps[8], "dotnet", "test", "PiPlay.sln", "--configuration", "Debug", "--no-restore");
        Assert.Equal(firstRoot,
            steps[8].GetProperty("environment").GetProperty("PIPLAY_DATA_ROOT").GetString());

        Assert.Equal("pwsh", steps[9].GetProperty("filePath").GetString());
        var buildArguments = Arguments(steps[9]);
