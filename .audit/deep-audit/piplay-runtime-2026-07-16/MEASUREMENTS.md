...
- Safety/cost/cleanup: local only; no URL, settings payload, or command line logged; one process inventory per two minutes; rows only while PiPlay is present; six-hour idle heartbeat; exact-root validation before creation; reparse roots rejected; retention failures logged. The protected installation grants modify only to Sev/SYSTEM/Administrators and read/execute to Users. A separate 15-minute watchdog complements the main task's logon and failure-restart behavior without retriggering a healthy long-running instance.
- Execution status: **installed; last live-verified healthy on 2026-07-18 Europe/Berlin** after the watchdog split and recovery validation. One logger PID remained unchanged when the watchdog was invoked against the healthy task; the status helper exposes both tasks, all matching PIDs, activity age, and Healthy/Stale/Duplicate/Missing classification.
...
- Interpretation/next action: pending. Analyze only after the acceptance threshold. On an anomaly, `deepen D-004` or `deepen D-008` and design a matched/active measurement before promoting anything. Quiet data can narrow general long-session retention only; it cannot close M-002's Focused comparison.
