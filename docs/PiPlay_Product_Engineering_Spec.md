...
Startup is single-instance per channel/session; a second normal launch activates the existing instance and hands off a supported YouTube target without contending for its WebView2 root. Exact `--help`, `-h`, and `/?` arguments show native usage and exit successfully before logging, instance election or handoff, settings, WebView2, or Source/Popout creation. Help wins over every other argument and creates no persistent application state. Outside help, the first argument accepted by `YouTubeUrlHelper.TryParse` is handed to normal startup verbatim; unsupported arguments are ignored.
