...
|---|---|---|
| 3.2 "Show popout" does not bring the popout back; duplicate risk | Toolbar button now flips label/tooltip/UIA name `Pop out video` ↔ `Bring video back`; the placeholder action uses the same `BringVideoBackAsync` path. The command captures fresh popout return state, closes the popout, and drives `ApplyReturnActionAsync`. Same-video return preserves paused/volume/mute/speed where the YouTube DOM allows it. | **Partially implemented in `fbe77c9`.** Returned-video navigation still needs post-navigation playback-state replay for paused/volume/mute/speed; tracked as Task 3b in `docs/superpowers/plans/2026-06-25-b25-full-review-followup.md`. Manual QA still needs real-page verification. |
