...
| 4 "Compact mode does not work" | "Compact" today is a **popout playback-surface** toggle in Settings (`SettingsWindow.xaml:199-203`), read only when building the next popout URL (`PlaybackModePolicy.cs`; `MainWindow.xaml.cs:824-827`). It never changes the main-window layout. | **Terminology mismatch** — the owner's main-window "Compact Mode" is net-new and does not exist; the shipped popout-compact is a different feature, itself **flagged for runtime QA**. |
...
|---|---|---|
| Accent/profile split | Implemented in working tree | The 2026-06-25 follow-up makes `Profile.AccentColor` an identity color, keeps `theme.accentColor` as the global app accent, shows the profile color as the selector frame/row marker, and makes Settings always edit the global accent. Optional active-profile popout border remains a future enhancement. Design: `docs/superpowers/specs/2026-06-25-profile-color-identity-and-accent-fill-design.md`; plan: `docs/superpowers/plans/2026-06-25-profile-color-identity-and-accent-fill.md`. |
...
| Borderless resize zones | Implemented in working tree | Previous implementation was `WindowChrome.ResizeBorderThickness="6"` on both primary windows. `REQ-WINDOW-02` now uses 10 DIP edge resize zones plus 32 DIP corner length via native hit testing, without adding a visible size grip or touch-first 40 x 40 target. The historical June 7 implementation notes were pruned after this retained summary and the current 2026-06-25 borderless records captured the useful facts. Manual DPI resize QA remains a release-candidate check. |
| Compact player sweep | Stages 1–4 shipped (merged to main 2026-06-10) | Compact mode is implemented as **shell mode**: mode policy (`PlaybackModePolicy` — durable `null`/`normal`/`compact` vocabulary, global/profile precedence, 480×270 minimum, mode→URL join, profile-override video-id gate), global/profile placement UI, the local `player.html` shell served from the `https://piplay.local/` WebView2 virtual host (allowlisted on the Popout Player only), and the YouTube IFrame-API host↔shell bridge (`PlayerShellBridge`/`PlayerShellProtocol`). **Stage 4 shipped (2026-06-10, live-smoked):** the compact error bar + in-place normal-page fallback — IFrame-API error, failed shell load, and ready-watchdog timeout all surface the bar; playing-state recovery auto-dismisses it; the fallback reopens the current target at the best-known timestamp. The overhaul Task 3 pass (2026-06-10) additionally keeps compact recommendation clicks inside PiPlay (in-place retarget) and makes return video-aware in both modes. The earlier Stage-1 direct embed is superseded; `BuildEmbedUrl` is kept as a reserved fallback tier. Account-backed live playback rows in `docs/QA_Checklist.md` remain release-candidate QA. Historical June 7/10 implementation notes were pruned after these facts were retained here, in `docs/CHANGELOG.md`, and in the QA checklist. |
...
| Compact-mode placement | Global player default (`PlayerSettings.CompactMode`, off by default) plus optional profile override (`Profile.Mode`: null/global, `normal`, `compact`; legacy `embed` normalizes to `compact`). |
