...
- **M-001 — unmeasured DOM execution-failure cost:** no controlled fault-injection result exists. With fresh authorization, compare exact-source Stable under persistent `ExecuteScriptAsync` failure with matched healthy playback. Record process-tree CPU, failed-call CPU, memory, log rate, dispatcher delay, and recovery latency. Confirm at sustained CPU increase `>=1` percentage point or failed-call work `>=5 ms CPU/s`; reject only below `0.5` points and `2 ms/s`, with stable memory and dispatcher delay.
...
- **M-003 — unmeasured WPF accent-preview cost:** no authorized interactive profile exists. With fresh authorization, measure 30 s hue, intensity, and mixed sweeps after warmup: apply p50/p95/p99, preview latency, UI CPU, allocation/GC, frame time, and resource replacements. Confirm if p95 apply exceeds `8.3 ms`, p95 preview exceeds `100 ms`, or slow frames increase by both `>=3` points and `>=5%`; reject only below `4 ms`, `50 ms`, `1` point, and `0.5 MB/s`, with no correlated GC.
- **M-004 — unmeasured Popout lifecycle settling:** no safe lifecycle driver exists; `scripts/Test-UiSmoke.ps1` does not drive Popout cycles. With fresh authorization and a safe driver, run separate Standard and Focused 50-cycle processes plus a 30–60 min navigation/resize/DPI soak. Record memory, handles, GDI/USER, HWNDs, threads, child processes, post-close CPU, and latency. Confirm at `>=0.5 MB/cycle` plus `>=25 MB`, `>=1 handle/cycle` plus `>=25`, sustained GDI/USER or child-process growth, or `>=25%` latency growth; reject only below `0.2 MB/cycle`, `20 MB`, `10` handles, and `10%` latency growth, with zero GDI/USER drift and a stable process plateau.
