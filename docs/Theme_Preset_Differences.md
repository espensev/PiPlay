...
- `LetterboxMixCeiling = 0.06`: `AccentLetterbox = Mix(#000000, Primary, 0.06 × reach)`. `BackgroundWashMixCeiling = 0.04`: `AppBackgroundWash = Mix(AppBackground, Primary, 0.04 × reach)`. `PopoutAccentEdge` is `AccentBorder` with alpha `255 × reach`; `ProfileRowWashAlpha` is the preset `SubtleAlpha`.
- `ApplyResolvedAccent` routes through `ApplyAccentEverywhere` to `ThemeResourceApplier.ApplyAccentOnly`; runtime replaces only changed frozen brush/color pairs. Source content and `SourcePlaceholder` plus the Popout background consume `AccentLetterbox`; the Source window and toolbar consume `AppBackgroundWash`; Settings keeps plain `AppBackground`. The Source title-wash end stop is `0.80`, and `TextPrimary` over `AppBackgroundWash` must reach `4.5:1` for every preset at intensity 100.
- Each profile row uses its own accent at preset `SubtleAlpha`, below hover/selection states; null or invalid row accents fall back to the plain surface. The Popout root uses a `1 px` inset `PopoutAccentEdge`; it must not disturb DWM corner styles or Win11 frame-hairline suppression, and nothing paints over WebView2 video.
